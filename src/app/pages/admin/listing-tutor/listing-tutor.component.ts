...
import { TutorListingsService } from '../../../core/services/tutor-listings/tutor-listings.service';
...
@Component({
  selector: 'vojvoj-listing-tutor',
  templateUrl: './listing-tutor.component.html',
  styleUrl: './listing-tutor.component.scss'
})
export class ListingTutorComponent implements OnInit {
  ...
  pageTitle: String = "List of Tutors";
  ...
  constructor(
    private tutorListingsService: TutorListingsService,
  ...
  async ngOnInit() {
    ...
    await this.tutorListingsService.getTutorListing(this.authToken)
      .then((resp) => {
  ...
  async updateisActive(curStatus: string, userid: number) {
    ...
    UserStatusObj.setUserId(userid);
    UserStatusObj.setCurrentStatus(curStatus);
    UserStatusObj.setRequestType("Tutor")
