...
export class LoginPages implements OnInit {
  ...
  constructor(
    private userLoginServices: UserLoginService,
  ...
  async loginValidation() {
    ...
    const respData = await this.userLoginServices
    ...
    if (respData.data.data.token) {
      ...
      this.sessionStorage.setAdminUserId(respData.data.data.result.waID);
      this.sessionStorage.setAdminUserName(
        respData.data.data.result.personName
      );
